...
public class SendRegisterVerificationCodeController {
    @Autowired
    private SendRegisterVerificationCodeService service;
    
    @PostMapping("/coderAuth/sendRegisterVerificationCode")
    public ResponseEntity<?> sendRegisterVerificationCode(@RequestHeader String id) {
        try {
            service.sendRegisterVerificationCodeService(id);
